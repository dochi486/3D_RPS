...
//
// Copyright 2013-2020 Jetro Lauha (Strobotnik Ltd)
...
// 2020-09-23, 1.8.4 - Added addDisableAdvertisingPersonalization, deprecated
//                     content experiments methods. Fixes for Unity 2020.2.
...
#       endif // web build
        ...
        sb.Append("&tid="); // Measurement/tracking/web property ID, required for all hit types
...
#       endif // debug logs
    ...
    //! (Category:General) Adds disabling of advertising personalization to a single event.
    /*! This method marks a single event as disabled for advertising personalization.
     *  This also applies to properties which would otherwise permit personalization of ads.
     *  An usage case example is a transaction that would be disabled for advertising
     *  personalization, so that it won't later be used for "past purchases" ads.
     * \sa setDisableAdvertisingPersonalization
     */
    public void addDisableAdvertisingPersonalization()
    {
        sb.Append("&npa=1");
    }
...
#       endif // debug warnings
     ...
     * \deprecated This has been removed from measurement protocol reference.
     */
    public void addExperimentID(string text)
    {
#       if GUA_DEBUG_WARNINGS
        Debug.LogWarning("GUA deprecation warning: Content Experiments have been deprecated");
#       endif // debug warnings
        string data = escapeString(text);
        sb.Append("&xid=");
        sb.Append(data);
...
#       endif // debug logs
    ...
    private byte[] gwrGet_bytes(GUAWebRequest gwr)
    {
        ...
        if (gwr.result != UnityWebRequest.Result.Success || gwr.downloadHandler == null)
            return new byte[0];
        #else
    ...
    private bool gwrGet_isError(GUAWebRequest gwr)
    {
        #if GUA_USE_WEBREQUEST
        #if UNITY_2020_2_OR_NEWER
        return gwr.result != UnityWebRequest.Result.Success || gwr.responseCode >= 400;
        #else
    ...
    private string gwrGet_errorString(GUAWebRequest gwr)
    {
        #if GUA_USE_WEBREQUEST
        #if UNITY_2020_2_OR_NEWER
        if (gwr.result != UnityWebRequest.Result.Success)
            return gwr.error;
        #else
