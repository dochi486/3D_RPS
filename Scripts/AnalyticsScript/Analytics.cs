...
//
// Copyright 2013-2018 Jetro Lauha (Strobotnik Ltd)
// http://strobotnik.com
// http://jet.ro
//
// $Revision: 1317 $
...
public class Analytics : MonoBehaviour
{
    ...
    string trackingID = "UA-xxxxxxx-x"; // 
...
#       if UNITY_3_5 || UNITY_4_0 || UNITY_4_1 || UNITY_4_2 || UNITY_4_3 || UNITY_4_5 || UNITY_4_6 || UNITY_4_7 || UNITY_5_0 || UNITY_5_1 || UNITY_5_2
        string sceneName = TG.getActiveSceneName();
#       else
...
#       endif
    ...
    void Awake()
    {
		...
		appVersion = PlayerPrefs.GetString("AppVersion", "1.0.0");
        ...
        if (gua.shouldWarnAboutUsingExampleTrackingProperty())
        {
            ...
            Text guiWarning = guiWarningGO.AddComponent<Text>();
            ...
            guiWarning.alignment = TextAnchor.UpperCenter;
...
#if UNITY_EDITOR
            sendSystemInfo = false;
#endif
            if (sendSystemInfo &&
                (useOfflineCache || gua.internetReachable) &&
                !PlayerPrefs.HasKey(prefKey))
            {
                GameObject go = GameObject.Find("GHGlobal");
                if(go)
                    go.SendMessage("SetFirstExcute");
...
#       endif
    ...
    {
#if UNITY_EDITOR
        // 에디터에서만 발생하는 아래 에러는 에러 수집에서 제외함.
        // (Assert)Material doesn't have a texture property '_MainTex':
        if( logString.Contains("_MainTex") )    
            return;
#endif
